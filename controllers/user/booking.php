    ...
    if (!$lapID || !$tgl || !$jamMulai || !$jamSelesai || !$durasi) {
        ...
        if ($cekKonflik->num_rows > 0) {
            ...
            $stmtBook = $conn->prepare("
                INSERT INTO booking
                    (kode_booking, user_id, lapangan_id, tanggal, jam_mulai, jam_selesai, durasi, total_harga, status)
                VALUES (?, ?, ?, ?, ?, ?, ?, ?, 'pending')
            ");
            $stmtBook->bind_param("siissssi", $kodeBooking, $user_id, $lapID, $tgl, $jamMulai, $jamSelesai, $durasi, $total);
